...
|---|---|---|---|---|
| `CVBuffer` attachments | `CVBuffer.h` | `CVBuffer`, `CVAttachmentMode`, typed attachment storage and propagation operations | Implemented | Portable behavior and Apple differential tests |
...
| Pixel buffer pools | `CVPixelBufferPool.h` | No declaration | Planned | Allocation-threshold and reuse tests |
| IOSurface-backed buffers | `CVPixelBufferIOSurface.h` | Portable storage lease boundary only | Adapter | Apple adapter conformance |
| Metal buffers and textures | `CVMetalBuffer*.h`, `CVMetalTexture*.h` | Opaque native-handle/lease direction only | Adapter | Cache lifetime and zero-copy projection tests |
...
storage. Future binary attachment values must retain a byte owner rather than
materializing payload bytes in an array.
